...
def run_dnn():
    train_means = np.random.normal(size=[100, 1])
    train_standard_deviations = np.random.gamma(shape=2, size=[100, 1])
    train_examples = np.random.normal(train_means, train_standard_deviations, size=[100, 10])
    train_labels = np.concatenate((train_means, train_standard_deviations), axis=1)

    test_means = np.random.normal(size=[1000, 1])
    test_standard_deviations = np.random.gamma(shape=2, size=[1000, 1])
    test_examples = np.random.normal(test_means, test_standard_deviations, size=[1000, 10])
...
losses = []
for _ in range(10):
    losses.append(run_dnn())
print('Mean DNN loss: {}'.format(sum(losses) / len(losses)))
